...
describe("calculator quote engine", () => {
  ...
  it("aplica beca fija aunque el promedio sea menor a 7", () => {
    const result = runQuoteEngine({
    ...
    expect(result.sinAccessToScholarship).toBe(false);
    expect(result.scholarshipSource).toBe("fixed");
    expect(result.scholarshipAmountMxn).toBe(1200);
    expect(result.totalMxn).toBe(1800);
  });
