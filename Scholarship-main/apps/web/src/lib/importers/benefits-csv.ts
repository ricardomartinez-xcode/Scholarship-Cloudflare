...
const HEADER_ALIASES = {
  ...
  businessLine: ["businessline", "lineanegocio", "linea"],
  ...
  for (let index = 1; index < rows.length; index += 1) {
    ...
    if (benefitType === AdminAdditionalBenefitType.first_payment) {
      ...
      if (firstPaymentAmountRaw && Number(firstPaymentAmountRaw) !== 0) {
        warnings.push(
          `Fila ${rowNumber}: first_payment_amount se ignoró porque benefit_type=${benefitType}.`,
        );
      }
    }
    ...
    const parsedRow: ParsedBenefitRow = {
      ...
      appliesToAll,
      campusIds: normalizedCampusIds,
      extraPercent: benefitType !== AdminAdditionalBenefitType.first_payment ? extraPercent : 0,
      firstPaymentAmount:
