...
const HEADER_ALIASES = {
  ...
  plan: ["plan"],
  tier: ["tier"],
  newPrice: ["newprice", "precio", "precio_base", "monto", "new_price"],
...
export async function preparePricesCsvImport(
  ...
  if (idxNivel < 0 || idxModalidad < 0 || idxPlan < 0 || idxNewPrice < 0) {
    throw new Error(
      "Faltan columnas obligatorias: nivel_key, modalidad_key, plan, new_price/precio_base.",
    );
  }
