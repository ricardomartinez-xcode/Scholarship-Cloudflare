...
import { buildLegacyDiscountedOverrideMap, legacyDiscountedOverrideKey } from "@/lib/base-price-overrides";
import {
  buildCampusAliases,
  type CampusIdentity,
...
export async function loadCanonicalFlatRulesPayload(sourceVersion = "legacy") {
  ...
  const overrideMap = buildLegacyDiscountedOverrideMap(overrides);
