...
export async function POST(
  ...
  const requestId = buildAdminRequestId("admin_benefits_import_rollback");
  try {
    ...
    await writeAdminAuditLog({
      ...
      entityId: sessionId,
      message: "Rollback lógico del draft de beneficios.",
    ...
    return adminApiSuccess(
      ...
      { message: "Rollback de importación de beneficios ejecutado." },
    );
  } catch (error) {
    logAdminApiFailure({
      requestId,
    ...
    return adminApiError({
      requestId,
      status: 500,
      errorCode: "BENEFITS_IMPORT_ROLLBACK_FAILED",
      error:
        error instanceof Error
          ? error.message
          : "No fue posible revertir la sesión de importación de beneficios.",
      recoverable: true,
    });
  }
}
