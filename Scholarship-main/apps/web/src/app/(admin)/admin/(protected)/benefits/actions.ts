  ...
  try {
    ...
    if (benefitType !== AdminAdditionalBenefitType.first_payment) {
      ...
      if (!Number.isFinite(extraPercent) || extraPercent <= 0) {
        return { ok: false, error: `${percentLabel} debe ser un número mayor que 0.` };
      }
      if (extraPercent % 5 !== 0) {
        return { ok: false, error: `${percentLabel} debe ser múltiplo de 5.` };
      }
      if (extraPercent > 100) {
        return { ok: false, error: `${percentLabel} no puede ser mayor a 100.` };
      }
    }
    if (benefitType === AdminAdditionalBenefitType.first_payment) {
      ...
      if (id) {
        await tx.adminAdditionalBenefit.update({
          ...
          data: {
            ...
            extraPercent:
              benefitType !== AdminAdditionalBenefitType.first_payment ? extraPercent : 0,
            firstPaymentAmount:
              benefitType === AdminAdditionalBenefitType.first_payment
                ? firstPaymentAmount
                : 0,
