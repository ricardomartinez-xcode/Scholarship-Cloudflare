...
function formatBenefitValue(benefit: Benefit) {
  ...
  if (benefit.benefitType === "fixed_scholarship") {
    return `Beca fija ${benefit.extraPercent}%`;
  }
...
export default function BenefitsClient({
  ...
  function startEdit(benefit: Benefit) {
    ...
    setExtraPercent(
      benefit.benefitType !== "first_payment" ? String(benefit.extraPercent ?? "") : "",
    );
    setFirstPaymentAmount(
  ...
  return (
    <section className="ui-card ui-card-pad">
                    ...
                    {importPreviewRows.slice(0, 30).map((row) => (
                      <tr key={`${row.rowNumber}-${row.action}`}>
                        ...
                        <td className="ui-cell-nowrap text-slate-100">
                          {row.benefitType !== "first_payment"
                            ? `${row.benefitType === "fixed_scholarship" ? "Beca fija " : ""}${row.extraPercent}%`
                  ...
                  <td className="ui-cell-nowrap text-right">
                    <div className="flex justify-end gap-2">
                      ...
                      >
                        ...
                        <button
                          type="submit"
                          className="whitespace-nowrap rounded-xl border border-red-500/30 bg-red-500/10 px-3 py-2 text-xs font-semibold text-red-100 transition hover:bg-red-500/15 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-500/30"
                        >
                          Eliminar
                        </button>
                      </form>
      ...
      <AdminDialogShell
        ...
        title="Beca o beneficio"
        description="Configura becas fijas, beneficios adicionales, primer pago, alcance y estado sin salir del módulo."
        size="xl"
      >
        ...
        <form onSubmit={handleSubmit} className="grid gap-4">
            ...
            {benefitType === "first_payment" ? (
              ...
              <div className="grid gap-2">
                <div id={percentId} className="text-sm">
                  {benefitType === "fixed_scholarship" ? "% de beca fija" : "% adicional"}
                </div>
                <SmartSelect
                  labelId={percentId}
          ...
          <label className="grid gap-2 text-sm">
            {benefitType === "first_payment"
              ? "Notas visibles del primer pago"
              : benefitType === "fixed_scholarship"
                ? "Notas de la beca fija"
                : "Notas"}
            <textarea
              name="notes"
              value={notes}
              ...
              placeholder={
                benefitType === "first_payment"
                  ...
                  : benefitType === "fixed_scholarship"
                    ? "Ej. Beca fija autorizada para este tipo de ingreso."
                    : "Opcional"
              }
            />
          </label>
