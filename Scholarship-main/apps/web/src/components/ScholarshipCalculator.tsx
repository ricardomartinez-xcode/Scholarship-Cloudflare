...
type BenefitBundle = {
  ...
  fixedScholarshipBenefit?: BenefitInfo | null;
...
const toQuoteApiResult = (result: SimulatorResultSnapshot): QuoteApiResult => ({
  ...
  fixedScholarshipPercent: result.fixedScholarshipPercent ?? 0,
  fixedScholarshipNotes: result.fixedScholarshipNotes ?? null,
  fixedScholarshipDuration: result.fixedScholarshipDuration ?? null,
  scholarshipSource: result.scholarshipSource ?? (result.sinAccessToScholarship ? "none" : "average"),
...
const baseFromRules = (rules: FlatRule[]) => {
  const inferred = rules
    .map((rule) => baseFromRule(rule))
    .filter((value): value is number => typeof value === "number" && Number.isFinite(value));
  if (!inferred.length) return null;
  return Math.max(...inferred);
};
...
export default function ScholarshipCalculator({
    ...
    const run = async () => {
      ...
      try {
        ...
        setBenefitBundle({
          ...
          fixedScholarshipBenefit: data.fixedScholarshipBenefit ?? null,
  ...
  const legacyCalculation = useMemo<Calculation | null>(() => {
    ...
    const fixedScholarshipBenefit = benefitBundle?.fixedScholarshipBenefit ?? null;
    const fixedScholarshipPercent = fixedScholarshipBenefit?.extraPercent ?? 0;
    const usesFixedScholarship = fixedScholarshipPercent > 0;
    const sinAccesoBeca = promedioNum < 7 && !usesFixedScholarship;
    let match = usesFixedScholarship || sinAccesoBeca ? null : findRuleForPromedio(candidatos, promedioNum);
 
    if (!usesFixedScholarship && !sinAccesoBeca && !match) {
    ...
    let porcentaje = usesFixedScholarship ? fixedScholarshipPercent : toNum(match?.porcentaje) ?? 0;
    if (!usesFixedScholarship && tipo !== "nuevo_ingreso") {
    ...
    if (typeof base !== "number") {
      base = baseFromRules(candidatos) ?? baseFromRule(match);
    }
    if (typeof base !== "number") {
    ...
    return {
      ...
      fixedScholarshipNotes: usesFixedScholarship ? fixedScholarshipBenefit?.notes ?? null : null,
      fixedScholarshipDuration: usesFixedScholarship ? fixedScholarshipBenefit?.duration ?? null : null,
      scholarshipSource: usesFixedScholarship
        ? "fixed"
        : sinAccesoBeca
          ? "none"
          : "average",
  ...
  const quoteCalculation = useMemo<Calculation | null>(() => {
    ...
    return {
      ...
      fixedScholarshipPercent: quoteResult.fixedScholarshipPercent,
      fixedScholarshipNotes: quoteResult.fixedScholarshipNotes,
      fixedScholarshipDuration: quoteResult.fixedScholarshipDuration,
      scholarshipSource: quoteResult.scholarshipSource,
  ...
  const currentSimulatorResult = useMemo<SimulatorResultSnapshot | null>(() => {
    ...
    return {
      ...
      fixedScholarshipPercent: calcOk.fixedScholarshipPercent ?? 0,
      fixedScholarshipNotes: calcOk.fixedScholarshipNotes ?? null,
      fixedScholarshipDuration: calcOk.fixedScholarshipDuration ?? null,
      scholarshipSource: calcOk.scholarshipSource ?? (calcOk.sinAccesoBeca ? "none" : "average"),
  ...
  const resultPanelScholarshipSource = currentSimulatorResult
    ? calcOk?.scholarshipSource ?? (resultPanelSinScholarship ? "none" : "average")
    : activeScenarioResult?.scholarshipSource ??
      (resultPanelSinScholarship ? "none" : "average");
  ...
  const resultPanelScholarshipLabel =
    resultPanelScholarshipSource === "fixed" ? "beca fija" : "beca";
  const resultPanelHeroCopy = resultPanelSinScholarship
    ? "Sin acceso a beca con el promedio capturado."
    : showResultPanelBenefits
      ? `Incluye ${resultPanelScholarshipLabel} de ${formatPercent(resultPanelScholarshipPercent)}. También aplica beneficio adicional de ${formatPercent(resultPanelBenefitPercent)}.`
      : `Incluye ${resultPanelScholarshipLabel} de ${formatPercent(resultPanelScholarshipPercent)}.`;
  ...
  const whatsappPreviewData = useMemo<WhatsappTemplatePreviewData>(() => ({
    ...
    scholarshipText: resultPanelSinScholarship
      ? "Sin acceso a beca"
      : `${resultPanelScholarshipLabel}: ${formatPercent(resultPanelScholarshipPercent)} (-${formatMoney(resultPanelScholarshipAmount)})`,
  ...
  const handleCopyQuoteImage = useCallback(async () => {
    ...
    try {
      ...
      const detailRows: Array<{
        ...
        {
          label: `${resultPanelScholarshipSource === "fixed" ? "Beca fija" : "Beca"} ${
              ...
              <div className="rounded-2xl border border-white/10 bg-slate-950/20 p-4">
                ...
                <div className="mt-3 grid gap-2.5 text-sm">
                  <div className="flex items-center justify-between">
                    <span className="text-slate-300">Precio base sin beca</span>
