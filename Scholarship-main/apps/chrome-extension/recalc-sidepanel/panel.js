document.addEventListener("DOMContentLoaded", () => {
  ...
  const state = {
    ...
    lastQuote: null,
    lastDraft: null,
    currentBenefits: { benefit: null, fixedScholarshipBenefit: null, firstPaymentBenefit: null },
  };
  ...
  async function loadBenefits() {
    ...
    if (!businessLine || !modality || !plan) {
      state.currentBenefits = { benefit: null, fixedScholarshipBenefit: null, firstPaymentBenefit: null };
    ...
    try {
      ...
      const fixedScholarship = data?.fixedScholarshipBenefit ?? null;
      const firstPayment = data?.firstPaymentBenefit ?? null;
      state.currentBenefits = { benefit, fixedScholarshipBenefit: fixedScholarship, firstPaymentBenefit: firstPayment };
      const parts = [];
      if (fixedScholarship?.extraPercent) parts.push(`Beca fija: ${percent(fixedScholarship.extraPercent)}.`);
      if (fixedScholarship?.duration) parts.push(`Duración beca fija: ${fixedScholarship.duration}.`);
      ...
      refs.benefitSummary.textContent = parts.length ? parts.join(" ") : "No hay becas o beneficios adicionales activos.";
    } catch {
      state.currentBenefits = { benefit: null, fixedScholarshipBenefit: null, firstPaymentBenefit: null };
      refs.benefitSummary.textContent = "No fue posible consultar beneficios para esta combinación.";
    }
  ...
  function basePriceFromRule(rule) {
    if (!rule) return null;
    const explicitBasePrice = Number(rule.basePriceMxn);
    if (Number.isFinite(explicitBasePrice)) return explicitBasePrice;
  ...
  function basePriceFromRules(rules) {
    const inferred = rules
      .map((rule) => basePriceFromRule(rule))
  ...
  function calculateLocalLegacyQuote(payload) {
    ...
    const fixedScholarshipBenefit = state.currentBenefits?.fixedScholarshipBenefit ?? null;
    const fixedScholarshipPercent = Number(fixedScholarshipBenefit?.extraPercent || 0);
    const usesFixedScholarship = fixedScholarshipPercent > 0;
    const sinAccessToScholarship = average < 7 && !usesFixedScholarship;
    let matchedRule = usesFixedScholarship || sinAccessToScholarship ? null : candidateRules.find((rule) => ruleContainsAverage(rule, average));
    if (!usesFixedScholarship && !sinAccessToScholarship && !matchedRule) {
    ...
    const basePriceMxn = basePriceFromRules(candidateRules) ?? basePriceFromRule(referenceRule);
    if (basePriceMxn === null) {
      return null;
    }

    const scholarshipPercent = sinAccessToScholarship
      ? 0
      : usesFixedScholarship
        ? fixedScholarshipPercent
        : Number(referenceRule.porcentaje || 0);
  ...
  async function calculateQuote() {
    ...
    try {
      ...
      const scholarshipLabel = data.scholarshipSource === "fixed" ? "beca fija" : "beca";
      ...
      if (data.sinAccessToScholarship) {
        ...
        refs.resultSummary.textContent = `Incluye ${scholarshipLabel} de ${percent(scholarshipPercent)}. También aplica beneficio adicional de ${percent(additionalBenefitPercent)}.`;
      } else {
        refs.resultSummary.textContent = `Incluye ${scholarshipLabel} de ${percent(scholarshipPercent)}.`;
      }
