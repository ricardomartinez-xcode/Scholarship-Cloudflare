# ReCalc Sender popup backend compatibility

This folder contains the ReCalc Sender popup bundle derived from the old `premiumsender.app` integration.
...
The compatibility implementation is intentionally permissive and compatibility-first. It lets the repointed extension connect to `https://recalc.relead.com.mx` without requiring the old `premiumsender.app` PHP backend.

Current license response:

```txt
license = totalaccess
license_key = totalaccess
plan = totalaccess
...
1. Replace the permissive `totalaccess` fallback in `buildLegacyLicense()` with real ReCalc auth/license validation when production licensing is ready.
