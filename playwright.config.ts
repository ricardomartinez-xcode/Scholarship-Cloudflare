...
export default defineConfig({testDir:"./apps/web/tests",testMatch:["**/*.spec.ts"],outputDir:"meta-review-audit/logs/playwright-results",timeout:60000,reporter:[["list"],["html",{outputFolder:"meta-review-audit/logs/playwright-html",open:"never"}],["json",{outputFile:"meta-review-audit/logs/playwright-report.json"}]],expect:{timeout:10000,toHaveScreenshot:{animations:"disabled",caret:"hide",maxDiffPixelRatio:0.01}},retries:process.env.CI?1:0,use:{baseURL,locale:"en-US",timezoneId:"America/Mexico_City",viewport:{width:1600,height:960},trace:"retain-on-failure",screenshot:"on",video:"on",ignoreHTTPSErrors:true,headless:process.env.PW_HEADLESS==="0"?false:true},webServer:shouldStartServer?{command:webServerCommand,url:baseURL,reuseExistingServer:!process.env.CI}:undefined,projects:[{name:"chromium",use:{...devices["Desktop Chrome"],...(chromiumExecutablePath?{launchOptions:{executablePath:chromiumExecutablePath}}:{})}}]});
