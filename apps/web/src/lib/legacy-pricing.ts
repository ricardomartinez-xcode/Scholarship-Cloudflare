...
export async function loadLegacyPricingSnapshot(): Promise<LegacyPricingSnapshot> {
  ...
  const flatRules: LegacyFlatRule[] = ruleRows.map((row) => {
    const key = `${row.programa}|${row.nivel}|${row.modalidad}|${row.plan}|${row.tier}`;
    return {
...
export async function computeLegacyScholarshipQuote(
  ...
  const scholarshipAmountMxn = basePriceMxn * (scholarshipPercent / 100);
  ...
  return {
    ...
    scholarshipAmountMxn: sinAccessToScholarship ? 0 : scholarshipAmountMxn,
