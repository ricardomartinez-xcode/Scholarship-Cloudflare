...
export async function POST(request: Request) {
  const requestId = buildAdminRequestId("admin_base_scholarships_import");
  try {
    ...
    if (!(maybeFile instanceof File) || maybeFile.size <= 0) {
      return adminApiError({
        ...
        error: "Debes subir un archivo CSV para importar % de beca por promedio.",
        recoverable: true,
      });
    }
    ...
    const prepared = await prepareBaseScholarshipsCsvImport({ file: maybeFile });
    const session = await prisma.adminImportSession.create({
      data: {
        ...
        fileName: `base-scholarships:${maybeFile.name}`,
    ...
    await writeAdminAuditLog({
      ...
      entityId: session.id,
      after: { kind: "base_scholarships", ...prepared.summary },
    ...
    return adminApiSuccess(
      ...
      { message: "Importación de % de beca por promedio validada." },
    );
  } catch (error) {
    logAdminApiFailure({
      requestId,
    ...
    return adminApiError({
      requestId,
      status: 500,
      errorCode: "BASE_SCHOLARSHIPS_IMPORT_VALIDATE_FAILED",
      error:
        error instanceof Error
          ? error.message
          : "No fue posible validar el CSV de % de beca por promedio.",
      recoverable: true,
    });
  }
}
