...
export async function POST(request: Request, context: { params: Promise<{ sessionId: string }> }) {
  ...
  try {
    ...
    if (!session || session.module !== AdminConfigModule.OFFER || !session.fileName?.startsWith("campuses:")) return adminApiError({ requestId, status: 404, errorCode: "SESSION_NOT_FOUND", error: "Sesión de planteles no encontrada.", recoverable: true });
    ...
    await markAdminImportSessionApplied({ sessionId, module: AdminConfigModule.OFFER, actor: auth.admin, result: summary, afterSnapshot: afterSnapshot as never, captureAfterSnapshot: false, source: AdminChangeSource.IMPORT, requestId });
