import { adminApiError, adminApiSuccess, buildAdminRequestId } from "@/lib/admin-api";
import {
  confirmOperationsAction,
  type OperationsActionId,
  type OperationsActionPayload,
} from "@/lib/agents/operations-assistant";

import { requireOperationsAssistantAccess } from "../_shared";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

const ACTION_IDS = new Set<OperationsActionId>([
  "create_audit_note",
  "review_offer_imports",
  "document_env_setup",
]);

export async function POST(request: Request) {
  const requestId = buildAdminRequestId("operations_assistant_action_confirm");
  const auth = await requireOperationsAssistantAccess(requestId, "action-confirm", {
    limit: 5,
    windowMs: 60_000,
  });
  if (!auth.ok) return auth.response;

  const body = (await request.json().catch(() => null)) as
    | {
        actionId?: OperationsActionId;
        confirmationText?: string;
        payload?: OperationsActionPayload;
      }
    | null;
  if (!body?.actionId || !ACTION_IDS.has(body.actionId)) {
    return adminApiError({
      requestId,
      status: 400,
      errorCode: "OPERATIONS_ASSISTANT_INVALID_ACTION",
      error: "Accion no soportada.",
      recoverable: true,
    });
  }

  try {
    const result = await confirmOperationsAction({
      actor: auth.admin,
      actionId: body.actionId,
      confirmationText: String(body.confirmationText ?? ""),
      payload: body.payload,
      requestId,
    });
    return adminApiSuccess(requestId, { result }, { status: 201 });
  } catch (error) {
    return adminApiError({
      requestId,
      status: 400,
      errorCode: "OPERATIONS_ASSISTANT_CONFIRMATION_REQUIRED",
      error:
        error instanceof Error
          ? error.message
          : "Confirmacion requerida para ejecutar la accion.",
      recoverable: true,
    });
  }
}
