...
export async function GET(
  ...
  try {
    ...
    const payload = await listInboxMessagesForUser(session.user.id, threadId);
    if (!payload) {
...
export async function POST(
  ...
  try {
    ...
    const message = await createInboxMessageForUser({
    ...
    return NextResponse.json(
      {
        error: error instanceof Error ? error.message : "Failed to create inbox message",
      },
      { status: 500 },
    );
  }
