...
export default function ImportacionesLayout({ children }: { children: ReactNode }) {
  const linkClass =
    "rounded-2xl border border-white/10 px-4 py-2 font-semibold text-slate-200 transition hover:bg-white/10";
  ...
  return (
    <div className="grid gap-4">
      <nav className="mx-6 mt-6 flex flex-wrap items-center gap-2 rounded-3xl border border-white/10 bg-slate-950/35 p-2 text-sm">
        <Link href="/admin/importaciones" className={linkClass}>
          Centro
        </Link>
        <Link href="/admin/importaciones/lotes" className={linkClass}>
          Importación masiva
        </Link>
        <Link href="/admin/importaciones/plantillas" className={linkClass}>
